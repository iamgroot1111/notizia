...
export default function ClientCases({ clientId }: Props) {
  ...
  // Einklappen der Fälle-Liste & aufgeklappter Fall für Sessions
  const [showCases, setShowCases] = useState(false)
  const [openCaseId, setOpenCaseId] = useState<number | null>(null)

  // Sessions pro Case (Cache)
  ...
  return (
    <div className={styles.caseBlock}>
      ...
      <div className={styles.card}>
        ...
        <div className={styles.rowGrid}>
          <div className={styles.kv}>
            ...
            <select
              value={pcat}
              onChange={(e) => setPcat(e.target.value as Case['problem_category'])}
            >
        ...
        <div className={styles.toolbar}>
          <button className={styles.btn} onClick={addCase} disabled={!ptext.trim()}>
            Fall anlegen
          </button>
      ...
      {/* Toggle für die Fälle-Liste */}
      <div className={styles.toolbar} style={{ marginTop: 8 }}>
        <button className={styles.btn} onClick={() => setShowCases(s => !s)}>
          {showCases ? `Fälle verbergen (${cases.length})` : `Fälle anzeigen (${cases.length})`}
        </button>
      </div>
            ...
            {cases.map(cs => (
              <li key={cs.id} className={styles.card}>
                <div className={styles.header}>
                  ...
                  <div className={styles.toolbar}>
                    <button className={styles.btn} onClick={() => toggleSessions(cs.id)}>
                ...
                {openCaseId === cs.id && (
                  <div style={{ marginTop: 8, display:'grid', gap:8 }}>
                    {/* Session-Form */}
                    <form
                      onSubmit={(e) => { e.preventDefault(); void addSession(cs.id) }}
                    ...
                    >
                      ...
                      <div className={styles.toolbar}>
                        <button className={styles.btn} type="submit">Sitzung hinzufügen</button>
                      </div>
                    </form>
